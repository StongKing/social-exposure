# -*- coding: utf-8 -*-
"""
Created on Sat Nov  8 22:21:39 2025
...
naics_map = {
    ...
    '813110':  'Religious Organizations_christian'
}
'''
...
# 保存结果文件
results_path = f"{cat_dir}/results_{city}_{naics_code}.csv"
results_df.to_csv(results_path, index=False)

print(f"\n结果已保存至: {results_path}")

# ====== 将最终 H_opt_df 保存为文件（无正则） ======
import pickle
outfile = os.path.join(cat_dir, f'H_opt_df_dynamic_{naics_code}.pkl')
with open(outfile, 'wb') as _f:
    pickle.dump(H_opt_df, _f)
print("Saved final H_opt_df to:", outfile)


# ====== 将最终 H_opt_df 保存为文件（无正则） ======
import pickle
outfile_no_regu = os.path.join(cat_dir, f'H_opt_df_no_regu_{city}_{naics_code}.pkl')
with open(outfile_no_regu, 'wb') as _f:
    pickle.dump(H_opt_df, _f)
print("Saved final H_opt_df (no regularization) to:", outfile_no_regu)
