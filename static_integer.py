# -*- coding: utf-8 -*-
"""
Created on Wed Aug 20 10:16:50 2025
...
selected_pois = poi_total_flow.sort_values(ascending=False).head(53).index.tolist()# 193
...
social_iter_weight.append((S_sub*A_sub).sum().sum())
...
factors = np.arange(0.01, 1.01, 0.01)
...
for factor in factors:
    ...
    # 设置目标函数
    obj = sum(c[i * len(selected_pois) + j] * H_vars[(i, j)] 
              for i in range(len(selected_cbgs)) for j in range(len(selected_pois)))
    solver.Minimize(obj)
    ...
    # 3. 流量守恒约束
    for i, cbg in enumerate(selected_cbgs):
        solver.Add(sum(H_vars[(i, j)] for j in range(len(selected_pois))) == A_sub.iloc[i, :].sum())

    # 4. 容量约束
    for j, poi in enumerate(selected_pois):
        solver.Add(sum(H_vars[(i, j)] for i in range(len(selected_cbgs))) <= beta * A_sub[poi].sum())
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        # 检查收敛性
        diff = np.linalg.norm(H_opt_df.values - A_sub.values)
        print(f"Percent {iteration + 1} %: Change in H = {diff}")
        ...
        cc = (S_sub*H_opt_df).sum().sum()
        social_iter_weight.append(cc)
...
# 1. 绘制 f_values_iter
axs[0, 0].plot(f_values_iter[:101], color=colors[0], linewidth=2, marker='^', markersize=4)
axs[0, 0].set_title('Function Values', pad=10)
axs[0, 0].set_xlabel('Iteration')
axs[0, 0].set_ylabel('F-value')
axs[0, 0].grid(True, linestyle='--', alpha=0.7)
...
# 2. 绘制 diff_iter
axs[0, 1].plot(diff_iter[:100], color=colors[1], linewidth=2, marker='o', markersize=4)
axs[0, 1].set_title('Difference between the traffic flow $\|H^{k+1}-H^{k}\|_2$', pad=10)
axs[0, 1].set_xlabel('Iteration')
axs[0, 1].set_ylabel('Difference')
axs[0, 1].grid(True, linestyle='--', alpha=0.7)
...
# 3. 绘制 distances_iter
axs[1, 0].plot(distances_iter[:101], color=colors[2], linewidth=2, marker='d', markersize=4)
axs[1, 0].set_title('Distance', pad=10)
axs[1, 0].set_xlabel('Iteration')
axs[1, 0].set_ylabel('Distance')
axs[1, 0].grid(True, linestyle='--', alpha=0.7)
...
# 4. 绘制 social_iter
axs[1, 1].plot(social_iter[:101], color=colors[3], linewidth=2, marker='s', markersize=4)
axs[1, 1].set_title('Social Exposure', pad=10)
axs[1, 1].set_xlabel('Iteration')
axs[1, 1].set_ylabel('Sum of Social Exposure Matrix')
axs[1, 1].grid(True, linestyle='--', alpha=0.7)
...
print(A_sub.sum().sum())
print(H_opt_df.sum().sum())

# 统计矩阵 A_sub 中非零元素的个数
non_zero_A_sub = (A_sub != 0).sum().sum()
print("A_sub 中非零元素的个数:", non_zero_A_sub)

# 统计矩阵 H_opt_df 中非零元素的个数
non_zero_H_opt_df = (H_opt_df != 0).sum().sum()
print("H_opt_df 中非零元素的个数:", non_zero_H_opt_df)

# 统计矩阵 S_sub 中非零元素的个数
non_zero_S_sub = (S_sub != 0).sum().sum()
print("S_sub 中非零元素的个数:", non_zero_S_sub)
...
# 保存结果文件
results_path = f"{cat_dir}/results_static_{naics_code}.csv"
...
import pickle
import os
outfile_no_regu = os.path.join(cat_dir, f'H_opt_df_static_{naics_code}.pkl')
with open(outfile_no_regu, 'wb') as _f:
    pickle.dump(H_opt_df, _f)
print("Saved final H_opt_df to:", outfile_no_regu)
