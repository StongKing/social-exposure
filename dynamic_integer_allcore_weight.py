# -*- coding: utf-8 -*-
"""
Created on Wed Jan 21 10:47:01 2026
...
# 设置类别和文件路径
city = 'boston'  # 城市
category = 'Religious_Organizations_catholic'
...
# 加载 CBG 收入水平数据
cbg_income_dist_df = pd.read_csv(f'matrices_A_D_S_Distribution_{city}_core/cbg_income_level_distribution_{city}_msa_core.csv', dtype={'GEOID': np.int64})
...
# non_zero_count = (poi_total_flow > 0).sum()
# poi_num = non_zero_count
...
social_iter_weight.append((S_sub*H_prev).sum().sum())
    ...
    for i, cbg in enumerate(selected_cbgs):
        for j, poi in enumerate(selected_pois):
            ...
            if restricted_mask.values.flatten()[k]:
                ...
                # ub = int(A_sub.values.max() * 2)  # 或者 A_sub.iloc[i, j] * beta
                # H_vars[(i, j)] = solver.IntVar(lb, ub, f"H_{i}_{j}")
...
poi_naics_map = {
    ...
    'Religious_Organizations_catholic': '813110'
}
...
# 4. 绘制 social_iter
axs[1, 1].plot(social_iter_weight, color=colors[3], linewidth=2, marker='s', markersize=4)
...
# # 保存图片
# fig_path = f"{cat_dir}/results_{city}_{naics_code}.png"
...
# ================= 保存最后一次优化得到的 H_opt_df =================
if status == pywraplp.Solver.OPTIMAL:
    hopt_final_path = f"{cat_dir}/H_opt_df_final_{city}_{naics_code}.pkl"
    H_opt_df.to_pickle(hopt_final_path)
    print(f"最后一次 H_opt_df 已保存至: {hopt_final_path}")
else:
    print("优化失败，未保存 H_opt_df")
...
results_df = pd.DataFrame({
    ...
    'social_iter_weight': pd.Series(social_iter_weight),       # 101维
    'diff_iter': pd.Series(diff_iter)            # 100维
})

# 保存结果文件
results_path = f"{cat_dir}/results_{city}_{naics_code}_weight.csv"
