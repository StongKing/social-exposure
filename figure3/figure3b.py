...
# ============================================================
# Load public Fig.3b data
# ============================================================

data_path = 'figure3b_data.csv'

flow_change_df = pd.read_csv(
    data_path,
    dtype={'GEOID': str}
)
...
# ============================================================
# Reconstruct original flow_change dictionary
# ============================================================
...
for k in ks_to_plot:
    ...
    if colname not in flow_change_df.columns:
...
ax0.set_title(
    'Boston MSA reallocation budget = 1%\n'
    '(zoom area outlined)',
    pad=8
)
...
for i, k in enumerate(
    ...
    # geopandas adds collections to ax.collections;
    # the latest Polygon collection is usually at the end.
    ...
    try:
        ...
        # fallback: do nothing
        # (map still shown without circular mask)

        pass


    # draw circular outline on top to show crop boundary
    # (in axes coordinates, but using data coords)

    circ_outline = mpatches.Circle(
        (
            cx,
            cy
        ),
    ...
    # set zoom extents to the square containing the circle
    # so circle is centered in subplot
...
cb.set_label(
    r'Reassigned visits per CBG, '
    r'$\frac{1}{2}\sum_j |H_{ij}(b)-F_{ij}|$'
)
